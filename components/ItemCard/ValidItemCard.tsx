...
const ValidItemCard = ({ post, index = 0 }: validItemCard) => {
    const { user } = useUser()
    ...
    const likeMutation = useMutation(api.functions.users.likeValidItem)
    const shareMutation = useMutation(api.functions.users.shareValidItem)
            ...
            >
                ...
                <a onClick={() => handleVideoClick()}
                    ...
                    <div className="relative w-full sm:h-32 bg-slate-100 rounded-lg overflow-hidden">
                        ...
                        {/* Center Play Icon */}
                        {post.sourceType === "video" && (<div className="absolute inset-0 flex items-center justify-center">
                    ...
                    <div className="flex flex-row items-center justify-between gap-3 sm:gap-0 pt-2 border-t border-slate-50">
                        ...
                        {post.authorName && post.authorName !== 'null' && (
                            ...
                            >
                                ...
                                <span className="text-xs sm:text-sm font-semibold text-slate-700 truncate">{post.authorName}</span>
                            </Link>
                        )}
